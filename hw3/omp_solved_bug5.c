...
* COMMENT: deadlock occurs becaue the first thread holding lock a and requesting for lock b, while the second thread holding the lock b and requesting lock a. I solved this by implementing the
2PL where each thread acquires two locks at the same time and only release them once that thread is done.
******************************************************************************/
#include <omp.h>
#include <stdio.h>
#include <stdlib.h>
